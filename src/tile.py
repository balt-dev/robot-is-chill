...
from .cogs.generator import CharacterGenerator
...
class TileSkeleton:
    ...
    @classmethod
    def parse(cls, possible_variants, string: str, rule: bool = True, macros: dict[str, str] = {}):
        out = cls()
        if string == "-":
            return out
        if rule:
...
def get_bitfield(*arr: typing.Iterable[bool]):
...
def handle_tiling(tile: Tile, grid, pos, tile_borders=False):
    ...
    fallback = constants.TILING_VARIANTS[get_bitfield(adj_r, adj_u, adj_l, adj_d, 0, 0, 0, 0)]
    ...
    adj_rd = adj_d and adj_r and is_adjacent(
        (w, x + 1, y + 1, z), tile, grid, tile_borders)
    tile.frame = constants.TILING_VARIANTS.get(get_bitfield(adj_r,adj_u,adj_l,adj_d,adj_ru,adj_lu,adj_ld,adj_rd), fallback)
...
class Tile:
    ...
    default_color: tuple[int, int] = (0, 3)
    empty: bool = True
    ...
    variants: dict[str: Variant, str: Variant, str: Variant] = None
    ...
    def prepare(cls, tile: TileSkeleton, tile_data_cache: dict[str, TileData], grid,
        ...
        try:
            ...
            value = cls(name=name, sprite=(metadata.source, metadata.sprite), tiling=metadata.tiling,
                        color=metadata.active_color, default_color=metadata.active_color,
            ...
            if name[:5] == "text_":
                ...
                value = cls(name=name, tiling=constants.TILING_CHAR, variants=tile.variants, empty=False, custom=True,
                            sprite=character[0], color=color, default_color=color)
            else:
    ...
    def __hash__(self):
        return hash((self.name, self.sprite if type(self.sprite) is tuple else 0, self.frame, self.fallback_frame,
                     ...
                     self.custom_style, self.palette, self.overlay, self.hue,
