...
def get_fish_xn_yn(source_x, source_y, radius, distortion):
    """Get normalized x, y pixel coordinates from the original image and return
    normalized x, y pixel coordinates in the destination fished image.
    ...
    """
    try:
        return source_x / (1 - (distortion * (radius**2))
                           ), source_y / (1 - (distortion * (radius**2)))
    ...
    for x in range(len(dstimg)):
        for y in range(len(dstimg[x])):
            ...
            try:
                ...
                # if new pixel is in bounds copy from source pixel to
                # destination pixel
