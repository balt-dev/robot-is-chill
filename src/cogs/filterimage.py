...
def apply_filterimage(img: Image.Image, fil: Image.Image, absolute: bool):
    ...
    if not absolute:
        samplecoordinates[:, :, 0] += np.arange(npfil.shape[1])
        samplecoordinates[:, :, 1] = (
            samplecoordinates[:, :, 1].T + np.arange(npfil.shape[0])).T
    samplecoordinates %= npimg.shape[:2]
    brightness = npfil[:, :, 2]
    alpha = npfil[:, :, 3]
    out[alpha > 0] = npimg[samplecoordinates[:, :, 1],
                           samplecoordinates[:, :, 0]][alpha > 0]
    out[:, :, :3] *= (brightness /
                      255).reshape(brightness.shape +
                                   (1,)).repeat(3, 2)
    out[:, :, 3] *= (alpha / 255)
