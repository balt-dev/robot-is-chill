...
class ContextBase:
    ...
    def __init__(self, *, bot: Bot, tile: RawTile, grid: RawGrid, index: GridIndex,
                 tile_data_cache: dict[str, TileData], flags: dict[str, Any]) -> None:
        self.bot = bot
        self.tile = tile
        self.grid = grid
        self.index = index
...
class VariantHandlers:
    ...
    def handle_tile(self, tile: RawTile, grid: RawGrid, index: GridIndex, tile_data_cache: dict[str, TileData],
                    **flags: Any) -> FullTile:
...
async def setup(bot: Bot):
    ...
    def pixelate(ctx: HandlerContext) -> TileFields:
        pixelate = [max(int(ctx.groups[0]), 1),
                    max(int(ctx.groups[1]), 1) if ctx.groups[1] != None else max(int(ctx.groups[0]), 1)]
    ...
    @handlers.handler(
        ...
        variant_hints={"rotaterand": "`rotaterand[/<bool>]` (Rotates the sprite a random number of degrees counterclockwise. The second boolean, defaulting to true, decides whether or not to expand the bounding box of the sprite.)"},
        variant_group="Filters"
    )
    def rotate(ctx: HandlerContext) -> TileFields:
        return add(ctx, 'angle', (rand.random() * 360, (ctx.groups[0] or 'true') == 'true'))
    ...
    def scale(ctx: HandlerContext) -> TileFields:
        n = float(ctx.groups[2]) / float(ctx.groups[3]) if ctx.groups[2] else float(ctx.groups[0]) / float(
            ctx.groups[1])
        return add(ctx, 'scale',
                   (max(min((float(ctx.groups[0]) / float(ctx.groups[1])), 8), 0.01), max(min(n, 8), 0.01)))
    ...
    def displace(ctx: HandlerContext) -> TileFields:
        try:
            d = ctx.fields.get("displace")
            return {'displace': [a + b for a, b in zip((0 - (
                        (int(ctx.groups[0]) / int(ctx.groups[1])) * constants.DEFAULT_SPRITE_SIZE), 0 - ((int(
                ctx.groups[2]) / int(ctx.groups[3])) * constants.DEFAULT_SPRITE_SIZE)), d)]}
        except TypeError:
    ...
    def warp(ctx: HandlerContext) -> TileFields:
        return add(ctx, 'warp', (
        (float(ctx.groups[0]), float(ctx.groups[1])), (float(ctx.groups[2]), float(ctx.groups[3])),
        (float(ctx.groups[4]), float(ctx.groups[5])), (float(ctx.groups[6]), float(ctx.groups[7]))))
